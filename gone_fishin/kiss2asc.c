...
if ((argc < 2) || (argc > 3))
    ...
    puts("    with extensions for timestamp frames a la gr-satellites");
    puts("Usage:");
...
if (setvbuf(infile, (char *)NULL, _IOFBF, 8096) ||
    setvbuf(outfile,(char *)NULL, _IOFBF, 8096))
...
if (len == 0)			/* Special handling for empty frames */
...
else if (len == 8)      /* Special handling for timestamp frames */
    {
    for (i=0; i<8; i++)
        {
        time <<= 8;
    ...
    seconds = time/1000;
    strftime(timebuf, sizeof(timebuf)-1, "%Y-%m-%d %H:%M:%S", gmtime(&seconds));
    fprintf(outfile, "Timestamp: %s.%03ld\n", timebuf, time%1000);
    }
